...
- **Live full app (Heroku):** https://glassbox-evidence-834d99a1d3b7.herokuapp.com
...
2. **Record the demo video** (~2–3 min) and paste the link. Use [../DEMO.md](../DEMO.md) as the run-sheet. Must play for a logged-out viewer.
3. **Export the deck to PDF** (and ideally PPTX) for the "Link to Presentation" / upload.
4. (Optional but strong) Set `GEMINI_API_KEY` on Heroku so the live dashboard's *Analyze* works for judges who click through; the **verifier + Prove already work without it.**
